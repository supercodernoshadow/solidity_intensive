...
describe('Variables', () => {
  ...
  describe('Example 5', () => {
    ...
    beforeEach(async () => {
      ...
      const DerivedContract = await ethers.getContractFactory('Variables7')
      derivedContract = await DerivedContract.deploy()
    })

    it('demonstrates internal variable visibility', async () => {
      const name3 = await derivedContract.getName3()
      expect(name3).to.equal('Name 3')
    })

    it('demonstrates public variable visibility', async () => {
      const name4 = await contract.name4()
      expect(name4).to.equal("Name 4")
    })
